...
```js
		// ===== 导出接口 =====
		export { compileAllTemplates,compile };

		// 保留直接执行功能
		if (process.argv[1] === fileURLToPath(import.meta.url)) compileAllTemplates();
```
---
...
```js
		...
		if (process.argv[1] === fileURLToPath(import.meta.url))  {
		  startServer().catch(error => {
		    console.error('服务器启动失败:', error), process.exit(1);
		  });
		}
```
---
...
>
	...
	### 作为模块使用
	```javascript
	import { startDevServer, compile } from '@flun/html-template';
