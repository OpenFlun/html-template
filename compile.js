...
const {
	...
	getIncludedFiles, loadUserFeatures, findEntryFile, templatesDir, staticDir, customizeDir, defaultPort
} = require('./services/templateService'), PK = require('./package.json'),
	util = require('util'), { exec } = require('child_process'), execPromise = util.promisify(exec),
	...
	checkUserRoutesExist = async () => {
		try {
			const featuresDir = path.join(CWD, customizeDir), files = await fsPromises.readdir(featuresDir);
	...
	mergeDependencies = async (hasUserRoutes) => {
		...
		try {
			...
			basePkg.author = userPkg.author || "", basePkg.license = userPkg.license || "ISC", userDeps = userPkg.dependencies || {};
		} catch (err) { }
		...
		const finalPkg = {
			name: "dist-server", version: "1.0.0", ...basePkg, main: "server.js", scripts: { "dev": "node server.js" },
			dependencies: mergedDeps, overrides: { "fast-xml-parser": "^5.3.4" }
		};
	...
	generateServerEntry = async (hasUserRoutes, entryFile) => {
		const expressInit = `const express = require('express'), path = require('path'), app = express(),
		...
		if (hasUserRoutes) {
			return `
			${expressInit}, fs = require('fs'), allRoutes = [],

			wrapAppMethods = app => {
				const originalMethods = {}, methodsToWrap = ['get', 'post', 'put', 'delete', 'patch', 'options', 'head', 'all'];
				methodsToWrap.forEach(method => {
					originalMethods[method] = app[method].bind(app);
					app[method] = function(path, ...handlers) {
						allRoutes.push({ method: method.toUpperCase(), path: path });
						return originalMethods[method](path, ...handlers);
					};
				});
				return app;
			},
			// 自定义路由加载（用户可在此添加登录校验等中间件）
			loadUserRoutes = () => {
			...
			// 若用户未定义根路由，则添加默认重定向
			if (!allRoutes.some(route => route.method === 'GET' && route.path === '/')) ${defaultRootRoute};
			app.listen(port, () => {
				console.log(\`\\n🚀 服务已启动: http://localhost:\${port}\`);
				console.log('📡 路由监控:'), printRoutes();
			});
		`.trim();
		}
		// 无用户路由时,静态文件服务器保持简单公开
		else {
			return `
			${expressInit};
...
const compileAllTemplates = async (options = {}) => {
	...
	try {
		...
		// 4. 检测是否存在用户路由,生成package.json内容,获取入口文件名生成 server.js 内容，并原子写入磁盘
