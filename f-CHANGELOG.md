# 变更日志
## [4.0.2] - 2026-04-27 16:39
### 优化:
1. 将用户自定义目录中的routes.js文件修改为路由入口,account.js文件将通过此文件顺序加载路由,避免加载顺序的不确定性;
2. 优化了模板引擎服务中,对写入文件的监听和加载自定义文件的判断;
3. 在静态目录中的auth.js和styling.js中增加了全局登录标志和检查:account(布尔值);
> ***其它一些细节优化略...;***
---
